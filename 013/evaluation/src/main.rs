...
/// If no configuration file is specified by the command line arguments,
/// this pathname is used.
const DEFAULT_CONFIG_FILE_PATHNAME: &str = "config.toml";
...
fn main() {
    ...
    let config =
        read_to_string(args.config_file_pathname).expect("Failed to load configuration file.");
    let config: Config = toml::from_str(&config).expect("Failed to deserialize.");

    // Reads the list of seeds.
    let seeds: Vec<usize> = read_to_string(&config.seeds_file)
    ...
    create_dir_all(Path::new(&config.output_folder)).expect("Failed to create output folder.");

    // Creates vis folder.
    create_dir_all(Path::new(&config.vis_folder)).expect("Failed to create vis folder.");
...
#[clap(author, version)]
struct Args {
    /// Pathname of the Configuration file for executable files and file input/output
    #[clap(short = 'c', long = "config", default_value_t = String::from(DEFAULT_CONFIG_FILE_PATHNAME))]
    config_file_pathname: String,
    ...
    output_results_file_pathname: String,
}

#[derive(Debug, Deserialize)]
struct ExecutableFiles {
    submission: String,
    vis: String,
}
...
fn run_submission(config: &Config, seed: usize) -> f64 {
    let input_file_path = Path::new(&config.input_folder).join(&format!("{:04}.txt", seed));
    let output_file_path = Path::new(&config.output_folder).join(&format!("{:04}.txt", seed));

    let input = read_to_string(&input_file_path).expect("Failed to load file.");

    let start_time_instant = time::Instant::now();

    let child = Command::new(&config.executable_files.submission)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn run_visualize(config: &Config, seed: usize) -> usize {
    let input_file_path = Path::new(&config.input_folder).join(&format!("{:04}.txt", seed));
    let output_file_path = Path::new(&config.output_folder).join(&format!("{:04}.txt", seed));
    let vis_file_path = Path::new(&config.vis_folder).join(&format!("{:04}.html", seed));

    let output = Command::new(&config.executable_files.vis)
        .args([
