...
fn main() {
    ...
    // let unique_graphs = Graph::create_unique_graphs(node_num);

    // println!("unique_graphs.len() = {}", unique_graphs.len());

    // for graph in unique_graphs.iter().take(10) {
    //     graph.show_edges();
    // }
    ...
    if noise_rate <= 1 {
        ...
        // with_noise::run2(noise_rate as f64 * 2.0);
        // with_noise::run_with_verbose_graph(8);
    }
}
...
pub mod graph {
    ...
    use crate::{calc_edge_domain, get_epsilon, get_noise_rate, union_find::UnionFind};
    ...
    impl Graph {
                    ...
                    if (bit >> edge_idx) & 1 == 0 {
                        if let Some(cur_graph) = &dp[bit] {
                            ...
                            if !labeled_graphs.contains_key(&next_graph) {
                                ...
                                dp[bit | (1 << edge_idx)] = Some(next_graph);
                                class_cnt += 1;
                            }
                        }
                    }
        ...
        pub fn calc_indirect_connect_num(&self, from: usize, to: usize) -> usize {
            (0..self.node_num())
                .filter(|&mid| self.is_connect(to, mid) && self.is_connect(mid, from))
                .count()
        }

        // pub fn divide_node_groups(&self, group_num: usize) -> Vec<Vec<usize>> {
        //     assert!(group_num <= self.node_num());

        //     let mut uf = UnionFind::new(self.node_num());

        //     // let mut used = vec![false; self.node_num()];

        //     let mut indirect_connect_num_list = (0..self.node_num())
        //         .flat_map(|from| {
        //             (0..self.node_num())
        //                 .map(|to| (from, to, self.calc_indirect_connect_num(from, to)))
        //                 .collect_vec()
        //         })
        //         .collect_vec();

        //     indirect_connect_num_list.sort_by_key(|x| Reverse(x.2));

        //     for (from, to, _) in indirect_connect_num_list {
        //         if uf.group_num() <= group_num {
        //             break;
        //         }
        ...
        //     // let group_size = self.node_num() / group_num;

        //     // let mut groups = vec![];

        //     // for base_node in 0..self.node_num() {
        //     //     let mut queue = VecDeque::from(vec![base_node]);
        //     //     groups.push(vec![base_node]);

        //     //     while let Some(cur_node) = queue.pop_front() {
        //     //         for next_node in 0..self.node_num() {
        //     //             if !used[next_node] && indirect_connect_num_list[cur_node][next_node] + indirect_connect_num_list[cur_node][next_node] {
        ...
        // pub fn shrink(&self, shrink_node_num: usize, threshold: usize) -> Graph {
        //     let mut shrink_graph = Graph::new(shrink_node_num);

        //     let node_groups = self.divide_node_groups(shrink_node_num);

        //     for large_from in 0..node_groups.len() {
        //         for large_to in (large_from + 1)..node_groups.len() {
        //             let edge_num: usize = node_groups[large_from]
        //                 .iter()
        //                 .map(|&from| {
        //                     node_groups[large_to]
        //                         .iter()
        //                         .filter(|&&to| self.is_connect(from, to))
        //                         .count()
        //                 })
        //                 .sum();

        //             if edge_num >= threshold {
        //                 shrink_graph.add_edge(large_from, large_to);
        //             }
        //         }
        //     }
    ...
    impl VirtualGraph {
        pub fn new(node_num: usize, node_size: usize) -> Self {
            let mut inner_graph = Graph::new(node_num * node_size);

            for node_idx in 0..node_num {
                let left = node_size * node_idx;
                let right = left + node_size;

                for from in left..right {
                    for to in (from + 1)..right {
                        inner_graph.add_edge(from, to);
                    }
                }
            }
        ...
        pub fn from_raw_graph(raw_graph: Graph, node_size: usize) -> Self {
            let node_num = raw_graph.node_num() / node_size;

            Self {
                inner_graph: raw_graph,
                node_num,
                node_size,
            }
        }

        pub fn from_graph(graph: &Graph, node_size: usize) -> Self {
            let mut virtual_graph = VirtualGraph::new(graph.node_num(), node_size);

            for from in 0..graph.node_num() {
                for to in (from + 1)..graph.node_num() {
                    virtual_graph.add_edge(from, to);
                }
            }
        ...
        pub fn virtual_node_num(&self) -> usize {
            self.node_num
        }
        ...
        pub fn add_edge(&mut self, from: usize, to: usize) {
            for i in 0..self.node_size {
                self.inner_graph
                    .add_edge(self.node_size * from + i, self.node_size * to + i);
            }
        }

        pub fn estimate_node_groups(&self) -> Vec<Vec<usize>> {
            // let adjust_matrix = (0..self.inner_graph.node_num())
            //     .map(|from| {
            //         (0..self.inner_graph.node_num())
            //             .map(|to| self.inner_graph.is_connect(from, to) as usize)
            //             .collect_vec()
            //     })
            //     .collect_vec();

            // // for row in &adjust_matrix {
            // //     eprintln!("# {}", join(row, ""));
            // // }

            // let mat_mul =
            //     |mat1: &Vec<Vec<usize>>, mat2: &Vec<Vec<usize>>, row: usize, col: usize| -> usize {
            //         (0..self.inner_graph.node_num())
            //             .map(|i| mat1[row][i] * mat2[i][col])
            //             .sum()
            //     };

            // let pow2_adjust_matrix = (0..self.inner_graph.node_num())
            //     .map(|row| {
            //         (0..self.inner_graph.node_num())
            //             .map(|col| mat_mul(&adjust_matrix, &adjust_matrix, row, col))
            //             .collect_vec()
            //     })
            //     .collect_vec();

            // let pow3_adjust_matrix = (0..self.inner_graph.node_num())
            //     .map(|row| {
            //         (0..self.inner_graph.node_num())
            //             .map(|col| mat_mul(&adjust_matrix, &pow2_adjust_matrix, row, col))
            //             .collect_vec()
            //     })
            //     .collect_vec();

            let count_mid_nodes = |from: usize, to: usize| {
                (0..self.inner_graph.node_num())
                    .filter(|&mid| {
                        self.inner_graph.is_connect(from, mid)
                            && self.inner_graph.is_connect(mid, to)
                    })
                    .count()
            };
            ...
            let mut strength_list = (0..self.inner_graph.edge_domain())
                .map(|_| {
                    let elem = (from, to, count_mid_nodes(from, to));

                    to += 1;
                    if to == self.inner_graph.node_num() {
                        from += 1;
                        to = from + 1;
                    }
            ...
            strength_list.sort_by_key(|x| Reverse(x.2));

            let mut uf = UnionFind::new(self.inner_graph.node_num());

            for (from, to, _strength) in strength_list {
                if uf.group_num() <= self.virtual_node_num() {
                    break;
                }

                if uf.size(from) + uf.size(to) < self.node_size {
                    uf.merge(from, to);
                }
            }
        ...
        // pub fn estimate_node_groups_2(&self) -> Vec<Vec<usize>> {
        //     let mut node_groups = (0..self.virtual_node_num())
        //         .map(|node_idx| {
        //             (0..self.node_size)
        //                 .map(|i| self.node_size * node_idx + i)
        //                 .collect_vec()
        //         })
        //         .collect_vec();

        //     let calc_group_weight = |group: &Vec<usize>| {
        //         let mut weight = 0;

        //         for i in 0..group.len() {
        //             for j in (i + 1)..group.len() {
        //                 weight += self.inner_graph.is_connect(group[i], group[j]) as usize;
        //             }
        //         }
        ...
        //     let mut sum_weight: usize = node_groups
        //         .iter()
        //         .map(|group| calc_group_weight(group))
        //         .sum();
        ...
        pub fn shrink(&self, allow_dec_rate: f64) -> Graph {
            let epsilon = get_epsilon();

            let node_groups = self.estimate_node_groups();

            // eprintln!("# node_groups = {:?}", node_groups);

            let threshold =
                (self.node_size as f64 * (1.0 - epsilon) * (1.0 - allow_dec_rate)).round() as usize;

            let count_inner_edge_num = |virtual_from: usize, virtual_to: usize| {
                let mut cnt = 0;

                for &from in &node_groups[virtual_from] {
                    for &to in &node_groups[virtual_to] {
                        if self.inner_graph.is_connect(from, to) {
                            cnt += 1;
                        }
                    }
                }
            ...
            let mut shrink_graph = Graph::new(self.virtual_node_num());

            for virtual_from in 0..self.virtual_node_num() {
                for virtual_to in (virtual_from + 1)..self.virtual_node_num() {
                    if count_inner_edge_num(virtual_from, virtual_to) >= threshold {
                        shrink_graph.add_edge(virtual_from, virtual_to);
                    }
                }
            }

            for from in 0..shrink_graph.node_num() {
                for to in 0..shrink_graph.node_num() {
                    if shrink_graph.is_connect(from, to) {
                        eprint!("1");
                    } else {
                        eprint!("0");
                    }
        ...
        // pub fn shrink2(&self) {
        //     let node_group_list = (0..self.inner_graph.node_num()).collect_vec();
        // }
    }
...
pub mod with_noise {
    ...
    use crate::{
        ...
        graph::{Graph, GraphPartInfo, VirtualGraph},
    ...
    pub fn run_with_verbose_graph(node_size: usize) {
        let graph_num = get_graph_num();
        ...
        // classified_graphs
        //     .iter_mut()
        //     .for_each(|graphs| graphs.sort_by_cached_key(|graph| max_edge_num(graph)));
        // classified_graphs.sort_by_cached_key(|graphs| max_edge_num(&graphs[0]));

        // let virtual_graphs = (0..graph_num)
        //     .map(|graph_idx| VirtualGraph::from_graph(&classified_graphs[graph_idx][0], node_size))
        //     .collect_vec();

        let transmitting_graphs = unique_graphs
            .iter()
            .take(graph_num)
            .map(|graph| VirtualGraph::from_graph(graph, node_size))
            .collect_vec();

        println!("{}", node_num * node_size);
        for graph in &transmitting_graphs {
            graph.inner_graph().show_edges();
        }

        // let mut labeled_graphs = HashMap::new();
        // for (class, graphs) in classified_graphs.into_iter().take(graph_num).enumerate() {
        //     labeled_graphs.extend(graphs.into_iter().map(|graph| (graph, class)));
        // }
        ...
        for _ in 0..QUERY_NUM {
            ...
            let received_raw_graph = Graph::from_chars(&hh, node_num * node_size);
            // print!("# ");
            // received_raw_graph.show_edges();
            let received_graph = VirtualGraph::from_raw_graph(received_raw_graph, node_size);
            let received_shrink_graph = received_graph.shrink(0.1);

            // print!("# ");
            // received_shrink_graph.show_edges();

            // println!("# node_num = {}", node_num);
            // println!(
            //     "# received_virtual_graph.node_num() = {}",
            //     received_virtual_graph.node_num()
            // );
            // println!(
            //     "# received_virtual_graph.total_edge_num() = {}",
            //     received_virtual_graph.total_edge_num()
            // );
            // println!(
            //     "# received_shrink_graph.node_num() = {}",
            //     received_shrink_graph.node_num()
            // );
            // println!(
            //     "# received_shrink_graph.total_edge_num() = {}",
            //     received_shrink_graph.total_edge_num()
            // );
            // println!("# labeled_graphs.len() = {}", labeled_graphs.len());

            if let Some(&label) = labeled_graphs.get(&received_shrink_graph) {
                eprintln!("# actual = {}", label);
                println!("{}", label.min(graph_num - 1));
                io::stdout().flush().unwrap();
            } else {
                // println!("# UNKNOWN");
                println!("0");
