...
pub mod graph {
    ...
    use crate::{get_noise_rate, union_find::UnionFind};
...
pub mod without_noise {
    ...
    use crate::{
        ...
        graph::{Graph, GraphIdentity},
        QUERY_NUM,
    };

    fn create_distinguishable_graphs(node_num: usize) -> Option<HashMap<GraphIdentity, Graph>> {
        io::stdout().flush().unwrap();
        ...
        for edges in 0..(1_u128 << edge_domain) {
            ...
            let graph_identity = GraphIdentity::new(&graph);

            if !distinguishable_graphs.contains_key(&graph_identity) {
                distinguishable_graphs.insert(graph_identity, graph);
            }
        }
    ...
    pub fn run() {
        ...
        let graph_identity_list = distinguishable_graphs
            .into_iter()
            .map(|(graph_identity, _)| graph_identity)
            .collect_vec();
        ...
        for _ in 0..QUERY_NUM {
            ...
            let received_graph_identity = GraphIdentity::new(&received_graph);

            let predicted_graph_idx = graph_identity_list
                .iter()
                .find_position(|&graph_identity| graph_identity == &received_graph_identity)
                .unwrap()
                .0;
...
pub mod strategy {
    ...
    pub fn pred_by_edge_num_2(interval_rate: f64) {
        ...
        let graphs = edge_num_list
            .iter()
            .map(|&edge_num| Graph::from_edge_num(edge_num, node_num))
            .collect_vec();
