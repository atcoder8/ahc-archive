...
use rect_join::{initialize_n_and_m, CompGridPoints, GridPoints};
...
#[allow(unused)]
fn strategy1(init_grid_points: GridPoints) -> GridPoints {
    let stop_watch = StopWatch::new();

    let mut best_grid_points = init_grid_points.clone();
    let mut used = HashSet::new();
    used.insert((
        init_grid_points.marked().clone(),
        init_grid_points.grid_lines().clone(),
    ));
    let mut heap = BinaryHeap::from(vec![CompGridPoints(init_grid_points)]);

    while let Some(CompGridPoints(curr_grid_points)) = heap.pop() {
        ...
        for selection in selections {
            ...
            let grid_state = (
                marked_grid_points.marked().clone(),
                marked_grid_points.grid_lines().clone(),
            );

            if used.contains(&grid_state) {
                continue;
            }

            used.insert(grid_state);

            if marked_grid_points.sum_weight() > best_grid_points.sum_weight() {
                best_grid_points = marked_grid_points.clone();
            }

            heap.push(CompGridPoints(marked_grid_points));
        }
    ...
    loop {
        let selections = best_grid_points.all_possible_selections();
        ...
        for selection in selections {
            if best_grid_points.selectable(selection) {
                best_grid_points.marking(selection);
            }
        }
    }
...
pub mod rect_join {
    ...
    impl GridPoints {
        ...
        pub fn search_straight_selection(&self, coord: Coord) -> Option<Selection> {
        ...
        pub fn search_diagonal_selection(&self, coord: Coord) -> Option<Selection> {
            if self.is_marked(coord) {
                return None;
            }
        ...
        pub fn all_possible_selections(&self) -> Vec<Selection> {
            (0..n())
                .flat_map(|x| (0..n()).filter_map(move |y| self.search_for_selection((x, y))))
                .collect()
        }
        ...
        pub fn selectable(&self, selection: Selection) -> bool {
            !self.marked[selection[0].0 * n() + selection[0].1]
        ...
        pub fn marked(&self) -> &FixedBitSet {
            &self.marked
        }
    ...
    #[derive(Debug, Clone)]
    pub struct CompGridPoints(pub GridPoints);
    ...
    impl PartialEq for CompGridPoints {
        fn eq(&self, other: &Self) -> bool {
            self.cmp_by_sum_weight(other) == Ordering::Equal
        }
    }

    impl Eq for CompGridPoints {}

    impl PartialOrd for CompGridPoints {
        fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
            Some(self.cmp_by_sum_weight(other))
        }
    }

    impl Ord for CompGridPoints {
