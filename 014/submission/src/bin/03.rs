...
use crate::{rect_join::CompGridPoints, time_measurement::StopWatch};
...
pub mod rect_join {
    ...
    impl CompGridPoints {
        ...
        #[allow(unused)]
        pub fn cmp_by_sum_weight_and_moves_num(&self, other: &Self) -> Ordering {
            match self.0.sum_weight.cmp(&other.0.sum_weight) {
                Ordering::Less => Ordering::Less,
                Ordering::Equal => self.0.moves_num().cmp(&other.0.moves_num()).reverse(),
                Ordering::Greater => Ordering::Greater,
            }
        }

        #[allow(unused)]
        pub fn cmp_by_moves_num(&self, other: &Self) -> Ordering {
            self.0.moves_num().cmp(&other.0.moves_num())
        }
    }

    impl PartialEq for CompGridPoints {
        fn eq(&self, other: &Self) -> bool {
            (self.0.sum_weight, self.0.moves_num()) == (other.0.sum_weight, other.0.moves_num())
        }
    }
